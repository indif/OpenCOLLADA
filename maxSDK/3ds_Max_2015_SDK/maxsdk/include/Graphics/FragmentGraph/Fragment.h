	...
	*/
	enum FragmentFlags
	{
		...
		*/
		...
		/** Indicate whether this fragment requires being evaluated once more.
			Note this flag is mainly used to indicate if a viewport redraw is 
			required.
		*/
		FragmentFlagsNeedsRedraw = 0x00000002,
		/** Used by a fragment graph to indicate whether the current structure of 
		a fragment graph has been changed.
		*/
		FragmentFlagsStructureChanged = 0x00000004,
		/** This flag will be set true if the last frame of a render sequence 
		requires being rendered. A fragment could cache its output if 
		FragmentFlagsRedrawEndSequence is true because no further redraw would 
		be required.
		*/
		FragmentFlagsRedrawEndSequence = 0x00000008,
	};
